...
namespace BlazorNotify.Utils
{
    ...
    {
        public static Result Encrypt(string userKey, string userSecret, string payload)
        {
            var userKeyBytes = userKey.ToBase64();
            var userSecretBytes =userSecret.ToBase64();
            var payloadBytes = Encoding.UTF8.GetBytes(payload);

            return Encrypt(userKeyBytes, userSecretBytes, payloadBytes);
        }

        public static Result Encrypt(byte[] userKey, byte[] userSecret, byte[] payload)
        {
